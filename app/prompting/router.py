...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
        "prompting/courses.html", {"request": request, "courses": curriculum}
    )
...
async def workflow_page(request: Request):
    ...
    return templates.TemplateResponse("prompting/workflow.html", {"request": request})
...
async def module_page(request: Request, module_id: str, submodule_id: int):
    ...
    response.set_cookie(
        "session_id", session_id, httponly=True, max_age=7200
    )  # 2 hours
    return response
...
async def chat(message: ChatMessage) -> JSONResponse:
    ...
    if session.current_module:
        module = next(
            (m for m in FULL_CURRICULUM if m["id"] == session.current_module), None
        )
        if module:
            ...
            if session.current_submodule:
                submodule = next(
                    (
                        ...
                        if s["id"] == session.current_submodule
                    ),
                    None,
                )
                if submodule:
                    lesson_context["lesson"] = (
                        f"{submodule['title']} - {submodule['description']}"
                    )
...
async def load_sample_document(request: Request):
    ...
    if not sample_filename:
        return JSONResponse(
            {"success": False, "error": "No sample document available for this lesson"}
        )
...
async def upload_file(file: UploadFile = File(...), session_id: str = FastAPIForm(...)):
...
async def summarize(request: SummarizeRequest) -> JSONResponse:
    ...
    try:
        summary, has_constraints = await generate_workspace_summary(
            request.prompt, session.uploaded_document
        )
        ...
        return JSONResponse({"summary": summary, "has_constraints": has_constraints})
...
async def summarize_stream(request: SummarizeRequest):
    ...
    async def generate():
        full_response = ""
        try:
            async for chunk in stream_workspace_response(request.prompt, document_text):
                full_response += chunk
                yield f"data: {json.dumps({'chunk': chunk, 'done': False})}\n\n"
...
async def submit_quiz(answer: QuizAnswer) -> QuizResult:
    ...
    if correct:
        ...
        if session.current_module and session.current_submodule is not None:
            session.mark_submodule_complete(
                session.current_module, session.current_submodule
            )
...
async def analyze_presentation(
    ...
    try:
        ...
        analysis = await generate_presentation_analysis(
            presentation_text, request.topic
        )
