    ...
    try:
        ...
        if not api_key:
            print_status(
                "Gemini API",
                "FAILED",
                "No API key found (GEMINI_API_KEY or GOOGLE_API_KEY)",
            )
            return False
        ...
        if response.parts:
            ...
            print_status(
                "Gemini API",
                "FAILED",
                f"No response received. Finish reason: {finish_reason}",
            )
            return False
...
async def test_perplexity_api():
    ...
    try:
        ...
        if not api_key:
            print_status(
                "Perplexity API", "FAILED", "No API key found (PERPLEXITY_API_KEY)"
            )
            return False
